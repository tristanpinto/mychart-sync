...
table of the most recent 14 days. This is the file the output directory surfaces when
contextual queries ask about diabetes data — daily MDs are too granular
to scan.

Idempotent full-overwrite from in-memory inputs. The all-time period is
optional: present only if the caller supplies a non-empty all-time
summary list (typically after a --bulk-import).
"""
...
def _render_recent_glance(summaries: list[DailyLoopSummary], n: int = 14) -> str:
    ...
    for s in recent:
        ...
        if s.n_hypo_level_2 > 0:
            ...
            if s.n_hypo_level_1 > s.n_hypo_level_2:
                hypos += f", L1×{s.n_hypo_level_1 - s.n_hypo_level_2}"
...
def render_loop_telemetry(
    ...
    included only when the data span is materially larger than 30 days — i.e.,
    after a `--bulk-import`. For typical daily incrementals (30-day rolling
    window) it would be redundant with Last 30 days.
    """
    if not summaries:
        return (
            "# Loop telemetry\n\n"
    ...
    sections = [
        "# Loop telemetry",
        "",
        f"_Auto-generated by chartstash from Tidepool. Anchor date: {today.isoformat()} ({person_tz})._",
